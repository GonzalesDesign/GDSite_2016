...
(function($, Edge, compId){
        ...
        (function(symbolName) {
         	...
         	window.scrollTo(0, 0); 
                ...
                var docWidth = $(document).width(); //Doc file width
                var docHeight = $(document).height(); //Doc file height
                var winWidth = $(window).width(); //Viewport Width
                var winHeight = $(window).height(); //Viewport Height
                var scrnWidth = screen.width; //Monitor Width
                var scrnHeight = screen.height; //Monitor Height	
			...
			audioBtn.css({"cursor": "pointer"});
         		...
         		//-------------( Function: Link opens another window )-------------//
				  function fAudio(elem){
							 sym.$("Sweet_as_Honey")[0].play();
					}
					//-------------( Arrow Cursor changed to Pointer )-------------//
				  audioBtn.css({"cursor": "pointer"});
					//-------------( Binding Button )-------------//
					audioBtn.bind ("click",function(){fAudio(audioBtn);});
         		*/
                ...
                /*
                var elemName, symElem, parentElem, positionStyle, xPos, yPos, zIndx;
                function fCreateChildSymbol(elemName, symElem, parentElem){ //, animTym, positionStyle, xPos, yPos, zIndx){
                	elemName = $(sym.createChildSymbol(symElem, parentElem).getSymbolElement());
                	 	//tMx.to(elemName, animTym, {css:{ position:positionStyle, x : xPos, y : yPos, zIndex : zIndx}, ease:easeSine});
                }
               fCreateChildSymbol(mS, "mainScreen", "Stage");//, 1, "fixed", 0, 0, 0);
               */
             	/*
		//mS.css({"position" : "relative", "overflow" : "visible", "left" : 0 + "px", "top" : 0 + "px"}); //Changed absolute to fixed inherit scroll relative
		//mS.css({"overflow" : "visible", "left" : 0 + "px", "top" : 0 + "px"});

		//tMx.to(mS, 1, {css:{ position:"fixed", left : 0, top : 0}, ease:easeSine});
		//----=( Set a reusable variable to reference the stage )=----//
		//var stage = sym.$('Stage'); 
		//----=( Set a reusable variable to reference the parent )=----//
		...
		*/
		...
		/*
		// var bgContainer = $(sym.createChildSymbol("rectContainer", "Stage").getSymbolElement());
		//var horLinesBackground = $(sym.createChildSymbol("horLines3Background", "Stage").getSymbolElement()); //horLines3Background horLine1Background

		//tMx.to(horLinesBackground, 0, {css:{ position:"fixed", x : 0, y : 100, zIndex : 200}});
		
		//var hLB = $(sym.createChildSymbol("horLines3Background", "Stage").getSymbolElement()); //horLines3Background
			//tMx.to(hLB, 2, {css:{ position:"fixed", x : 200, y : 200, zIndex : 200}});
		*/
		/*
		var aHorLines = new Array();
		var horLinesXSpaces;
		var horLinesYSpaces;		
		for(var j=0; j<=10; j++){
			horLinesXSpaces = j *200+100;
			horLinesYSpaces = j *20+200
			console.log("horLinesYSpaces: " + horLinesYSpaces);
			//tMx.to(horLinesBackground, 0, {css:{ position:"fixed", x : 0, y : 100+j, zIndex : 200}});
			//horLinesBackground.css({"position" : "fixed", "left" : "100px", "top" : 100 * j + 10+"px"});
			//---------===( Create mB children buttons )===---------//
			//var hLB = sym.createChildSymbol("horLines3Background", "Stage"); //btn is the symbol with circle, circleBtn, textButton symbols in it
			var hLB = $(sym.createChildSymbol("horLines3Background", "Stage").getSymbolElement());
			...
			aHorLines[aHorLines.length] = hLB; 
			tMx.to(aHorLines[j], 1, {css:{ position:"fixed", x : horLinesXSpaces, y :horLinesYSpaces, zIndex : 200}}); 
			console.log("aHorLines[j]: "+aHorLines[j])
		}
		*/
		...
		//colrSqrRectangle.css({opacity:0});
		//tMx.to(colorSquareContainer, 0, {x:0, y:0, scaleX: 2, scaleY:3, backgroundColor:"#FF0000", zIndex : -10, opacity:.5, ease:Power2.easeOut});
	         			//colorSquareContainer.css({x:0, y:0, scaleX: 2, scaleY:3, zIndex : -10, opacity:.5});
		//-----( Solid Background )-----//
		//var colorSquareContainer = $(sym.createChildSymbol("colorSquareContainer", rectContainer).getSymbolElement()); 
                 ...
                 //var bgContainer = 
                 //sym.getSymbol("rectContainer").$("Rectangle").css("background-image","url(images/wall_red.gif)","background-repeat","repeat");
                 //sym.getSymbol("WelcomeBtn").$("Welcome").css("color","blue");
                 
                 //sym.$("rectContainer").css("background-image","url(images/wall_red.gif)","background-repeat","repeat");
                 //sym.getSymbol("#Stage").$("rectContainer").css("background-image","url(images/wall_red.gif)","background-repeat","repeat");//,"zIndex","200px");
                 
                 //sym.getSymbol('symbolName').$('divName').css(...) Rectangle
                 
                 //sym.$("bio").css("text-align","justify");
                 
                 //var bgContainer.css({"position" : "fixed", "left" : -bgLeft+"px", "top" : "0px", "width" : winWidth+bgLeft+"px", "
                 
               // var sB = $(sym.lookupSelector("siteBackground")).css("background-image","url(images/horLine2.gif)");  //wall_red horLine2
                //	siteBG.css({"position" : "fixed", "left" : -bgLeft+"px", "top" : "0px", "width" : winWidth+bgLeft+"px", "height" : winHeight+"px"});
                	
                /*
                $("body").css("background-color","#4f2100"); 

		var rWBG = $(sym.createChildSymbol("redWallBG", "Stage").getSymbolElement());
		//rWBG.css({"position" : "fixed", "left" : 0 + "px", "top" : 0 + "px", zIndex:1});
		tMx.to(rWBG, 0, {css:{ position:"fixed", width: winWidth, x : 0, y : 0, zIndex : -10}}); 

                //---------===( background-attachment : fixed IS NOT WORKING so I made one )===---------//
                var siteBG = $(sym.lookupSelector("siteBackground")).css("background-image","url(images/wall_red.gif)");
                	//siteBG.css({"position" : "fixed", "left" : -bgLeft+"px", "top" : "0px", "width" : winWidth+bgLeft+"px", "height" : winHeight+"px"});
                        //siteBG.css('transform', 'translate(0 500)');

                fTransform(siteBG, "0 500", 1);
		*/

         	 //----=( borderLeft: Frame decoration )=----//
         	/*
         	var bdrLft = $(sym.createChildSymbol("verticalBorder", "Stage").getSymbolElement());
         		var bdrWidth = $(bdrLft).width(); //Getting the width of the vertical border
         		console.log("bdrWidth: " +bdrWidth)
         		tMx.to(bdrLft, 0, {css:{ position:"fixed", x : -bdrWidth, y : 0, zIndex : 100}}); 

         	  var bdrRt = $(sym.createChildSymbol("verticalBorder", "Stage").getSymbolElement());
         	 tMx.to(bdrRt, 0, {css:{ position:"fixed", x : stageWidth, y : 0, zIndex : 100}}); 

         	 var borderLeft;
		borderLeft = sym.getSymbol("Stage").$("borderLeft");
		//borderLeft.css({"position" : "fixed", "height" : winHeight+"px", zIndex:100});
		tMx.to(borderLeft, 0, {css:{ position:"fixed", x : 100, y : 0, zIndex : 100, boxShadow:"-14px 0px 14px rgba(0, 0, 0, 0.5)"}}); 
		//tMx.to(borderLeft, 0, {boxShadow:"-14px 0px 14px rgba(0, 0, 0, 0.5)"});

		//----=( borderRight: Frame decoration )=----//
		var borderRight;
		borderRight = sym.getSymbol("Stage").$("borderRight");
		borderRight.css({"position" : "fixed", "height" : winHeight+"px", zIndex:100});
		//tMx.to(borderRight, 0, {boxShadow:"14px 0px 14px rgba(0, 0, 0, 0.5)"});
		//tMx.to(borderRight, 0, {css:{ position:"fixed", x : stageWidth/3, y : 0, zIndex : 100, boxShadow:"-14px 0px 14px rgba(0, 0, 0, 0.5)"}}); 
		*/
			...
			*/
                        ...
                        borderLeft = sym.getSymbol("Stage").$("borderLeft");
                        //borderLeft.css({"position" : "fixed", "height" : winHeight+"px", zIndex:100});
                        tMx.to(borderLeft, 0, {css:{ position:"fixed", x : 0, y : 0, zIndex : 100}}); 
                        tMx.to(borderLeft, 0, {boxShadow:"-14px 0px 14px rgba(0, 0, 0, 0.5)"});
                        //----=( borderRight: Frame decoration )=----//
                        borderRight = sym.getSymbol("Stage").$("borderRight");
                        borderRight.css({"position" : "fixed", "height" : winHeight+"px", zIndex:100});
                        ...
                        */
                        ...
                        //var desiredWidth = Math.round(winWidth * 1);
			...
			//colorSquareContainer.css({"left" : 0, "top" : topBackgrndYPos, zIndex : -10});  
			//console.log("topBackgrndYPos: " + topBackgrndYPos);

			
			if(winWidth < 750){
				tMx.to(audioBtn, 1, {css:{ position:"fixed", x : -20, y : topBackgrndYPos-(150*resPerc), scale:.65, opacity:.75, zIndex : 150}});//Positioning the audio controller based on the window size. 
			 } else {
				tMx.to(audioBtn, 1, {css:{ position:"fixed", x : 20, y : topBackgrndYPos-(100*resPerc), scale:1, opacity:.75, zIndex : 150}});//Positioning the audio controller based on the window size. 
			 }
                      ...
                      //  tMx.to(mB, 1, {css:{ position:"absolute", x : 0, y : mBYPosRes, scale:(mBScale * resPerc), zIndex:10}, ease:easeSine});

                       //  tMx.to(mB, 1, {css:{ position:"fixed", x : 0, y : mBYPosRes, scale:mBScale, zIndex:10}, ease:easeSine});

                       //var s2ScrnTopPos = s2.position().top; 
                       /*
                        if(mB.position().top <= 0){
                        	 tMx.to(mB, 1, {css:{ position:"fixed",  y : 0}, ease:easeSine});
                        }
                        */
				...
				sym.$(btnText).html(aMenuTitles[symbolInTheArray.getVariable("id")]);
			 }
			 */
			 /*
                        console.log("-------------------Resize Percentage Start:");
			console.log("winWidth:",winWidth);
			console.log("stageWidth:",stageWidth);
			console.log("resPerc:",resPerc);
			console.log("mBScale * resPerc:",(mBScale * resPerc));
			console.log("-------------------Resize Percentage End:");
			console.log("");
			console.log("");
			console.log("");
			*/
			...
			//$("#Stage").css("margin", "auto");
                        ...
                        if(resPerc >= 1){
                                resPerc = 1;
                                //mS.css('transform', 'scale(' + resPerc + ')');
                                 fTransform(mS, "0 0", resPerc);
                                //----=( CENTERING STAGE )=----//
                                $("#Stage").css("margin", "auto");
                                //console.log("resPerc:",resPerc);
                        } else {
                                //----=( On viewport resize, transform Main Screen from the top left corner )=----//
                                fTransform(mS, "0 0", resPerc);
         	...
         	//------------------------( SCREENS_VARIABLES )------------------------//
                ...
                //var scrollDwnSymbl = sym.getSymbol(s1).$("scrollDwnIcon");
                //var scrllDwnIcn = sym.getSymbol(scrollDwnSymbl).$("scrollDownIcon"); // Can't apply shadow to the png image
                
                // var scrllDwnIcn = $(sym.createChildSymbol("scrollDownIcon", "Stage").getSymbolElement());                 
                //var scrllDwnIcn = sym.getSymbol(s1).$("scrollDownIcon");
                	//tMx.to(scrllDwnIcn, animTym, {css:{ position:"absolute", top: 640}, ease:easeSine});
                	//tMx.to(mB, 1, {css:{ position:"absolute", x : 0, y : mBYPosRes, scale:(mBScale * resPerc), zIndex:10}, ease:easeSine});
                ...
                var s6 = sym.getSymbol(mS).$("s6");
                       ...
                       // s1Desc.css({"position" : "absolute", "left" : 0 + "px", "top" : -450 + "px"});
                        ...
                        ///////////////////////////////////////////////////////////////////
                        //s3a.css({"position" : "absolute", "left" : 1366 + "px", "top" : 0 + "px"});
                        //scrllDwnIcn is a png and shadow applies to rect
                        /*
                        tMx.to([s1, s2, s3, s4, s5, s6, s3a, s3Txt, s3b, s3c, s3d, s3e, s3f, s3g, s4a, s4b, s4c, s4d, s5Txt, s6a, s6b, s6c, s6d], 0, {boxShadow:"0px 0px 36px rgba(0, 0, 0, 0.5)"});
                        tMx.to([s3a, s3Txt, s3b, s3c, s3d, s3e, s3f, s3g, s4b, s4c, s4d, s5Txt], 0, {css:{ left: stageWidth}, ease:easeSine});
                        tMx.to([s6b, s6c, s6d], 0, {css:{ top: 768}, ease:easeSine});
                        tMx.to([s1Desc, s3Title, s3Descript, s4Title, s4Descript, s5Title, s5Descript, y5Title, y5Descript, tYTitle, tYDescript], 0, {textShadow:"2px 2px 2px rgba(0, 0, 0, 0.75)"});
			*/
//------------( LoadMainMenu_Buttons )------------//
               ...
               function fAddEvntListnr(currentTarget){
                ...
                function fLoadNewSet(elem, symbolInTheArray){
                	...
                	//console.log("currentSelectedButton:",currentSelectedButton);
                        ...
                        //window.scrollTo( 0, aBookmarkPos[symbolInTheArray.getVariable("id")])
                        //fAddEvntListnr(aBookmarkPos["2"]); 
                ...
                $.each(aBtns, function( count, symbolInTheArray ){
                	//currentSelectedButton = symbolInTheArray;
                        //---------===( jQuery Reference to the item. Not necessary )===---------//
                        //var btn = $(symbolInTheArray);
                        //console.log("btn: "+btn);
		...
		//tMx.to(mB, 1, {css:{ position:"fixed", x : 0, y : mBYPos, scale:mBScale, zIndex:10}, ease:easeSine}); //Menu buttons scaling //40
			...
			circleBtn.css({opacity:.5 });                                
                        if(rO == true){ //onRollOut
                        	if(winWidth < 1000){ //If the browser window is smaller than 1k then don't display menu text except when rolled over.
                                ...
                                //sym.$(btnText).html(aMenuTitles[2]);
                                //To fix: On rollout turn off text except the section selected. 
                                /*
                                for(var i = 0; i<aBtns.length; i++){
					//fBtnStatesOff(aBtns[i]); //Turn off all buttons
					sym.$(btnText).html("");
				}
                                sym.$(btnText).html(aMenuTitles[symbolInTheArray.getVariable("id")]);
                                */
				...
				*/
                               ...
                               // var selectdBtn = sym.$(btnText).html(aMenuTitles[symbolInTheArray.getVariable("id")]);
                                //var selectdBtn = aMenuTitles[symbolInTheArray.getVariable("id");
                                //console.log("selectdBtn: "+selectdBtn)
                                //selectdBtn.css({opacity:1 });
            ...
            ////////////////////////////////////////////////////////////////////////////////SCROLL TRIGGER START
         	...
         	var yOffst2;
         	function parallax(){
         		//e.preventDefault(); //THIS WAS ADDED LATER!
         		yOffst2 = window.pageYOffset/10;
         		yOffst3 = window.pageYOffset/1;
         		//console.log("yOffst2:", yOffst2 );
         		//console.log("yOffst3:", yOffst3 );
         		scrollAmount = $(window).scrollTop()*2.5;
         		//console.log("scrollAmount:", scrollAmount );
         		//var yPosConfigurator = (travelHeight)
         		var yPosNeg100 = (-100 - scrollAmount);
         		var yPosNeg384 = (-384 - scrollAmount);     

         		//--------------------====( Main Screen and Title : Keeping it still )====--------------------//
         		//tMx.to(mS, 0, {css:{ top : scrollAmount}, ease:easeSine});
         		//tMx.to(mS, 0, {css:{ top : scrollAmount}, ease:easeSine}); //Screen 1: Still
         		//tMx.to(sT, 0, {css:{ top : 0}, ease:easeSine});
         		//tMx.to(mB, 0, {css:{ top : scrollAmount}, ease:easeSine}); //Menu Buttons: Still
         		//tMx.to([siteBG, borderLeft, borderRight], 0, {css:{ top : scrollAmount}, ease:easeSine}); //Background: Still
				...
				tMx.to([s4a, s4b, s4c, s4d], animTym, {css:{ left: stageWidth}, ease:easeSine}); //Mahogany screen stay all the way to the right
         		...
         		//} else if (scrollAmount >= travelHeight*32 && scrollAmount <= travelHeight*34){
         			...
         			tMx.to(s5, animTym, {css:{ top : (aYPos[34] - scrollAmount)}, ease:easeSine}); //yPos13056
         				...
         				tMx.to(s5ClipCrib03b, animTym, {css:{clip:"rect(0px,"+ clipLeft + ", " + aYPos[2] + "px," + clipRight+")"}, ease:easeSine}); //Clip rectangle mask	
         		...
         		//} else if (scrollAmount >= travelHeight*34 && scrollAmount <= travelHeight*39){
         			...
         			tMx.to([s2, s3, s4, s5], animTym, {css:{ top : 0}, ease:easeSine}); // pause
         				...
         				tMx.to(s5ClipCrib03b, animTym, {css:{clip:"rect(0px,"+ clipLeft + ", " + aYPos[2] + "px," + clipRight+")"}, ease:easeSine});		
      ...
      Symbol.bindSymbolAction(compId, symbolName, "creationComplete", function(sym, e) {
      	//-----( SoundCloudSetting )-----//
      	/*
      	var soundCloudContainer = $(sym.createChildSymbol("rectContainer", "Stage").getSymbolElement());   
      		soundCloudContainer.css({"position" : "fixed", "left" : "50px", "top" : "400px", "height":"100px", "width":"100px", zIndex:300}); 
	//var soundCloudID = soundCloudContainer.setVariable("id", i);
         sym.$(soundCloudContainer).append('<iframe width="100%" height="450" scrolling="no" frameborder="no" src="https://soundcloud.com/glennmorrison/mendelssohn-song-without-words-op102-no4-performed-on-a-bechstein?in=lloyd-gonzales/sets/classical&auto_play=false&hide_related=false&visual=true"></iframe>');
	*/
      });
      //Edge binding end
	/*
